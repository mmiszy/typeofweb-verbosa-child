...
function typeofweb_generate_jobs_html($jobs) {
    ...
    foreach($jobs as $job) {
        ...
        $return .= '<td><a href="' . esc_attr($job->permalink) . '" itemprop="url" target="_blank" rel="noopener nofollow"><h2 itemprop="title">' . esc_html($job->title) . '</h2></a></td>';
        $return .= '<td><h3 itemprop="hiringOrganization"><span itemprop="name">' . esc_html($job->company) . '</span></h3></td>';
        $return .= '<td class="visuallyhidden" itemprop="description">' . esc_html($job->description) . '</td>';
    ...
    if (!is_wp_error($rss)) {
        ...
        foreach ($rss_items as $item) {
            ...
            $job->tags = implode(
                ' ',
                array_map(
                    function ($tag) { return '<span>' . esc_html(trim($tag)) . '</span>'; },
                    explode(
                        ',',
                        $item->get_tags()
                    )
        ...
        return typeofweb_generate_jobs_html($jobs);
    }
...
function typeofweb_add_jobs_shortcode( $atts ) {
    ...
    return typeofweb_remoteok_jobs();
}
